...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 11 08:10:23 2025
...
for ind, row in big_df.iterrows():
    print('-------------')
    print(f'FILE {ind} out of {len(filelist)}')
    print('-------------')
    file=row['filename']
    print('trying', file)
    try:
    ...
    '''
    if Time(f"{hdr['DATE-OBS']}T{hdr['TIME-OBS']}") < Time('2008-06-01T00:00:00'):
        print('skipping ',file)
        continue
    if Time(f"{hdr['DATE-OBS']}T{hdr['TIME-OBS']}") > Time('2008-08-01T00:00:00'):
        print('skipping ',file)
        continue
    '''
...
big_df.to_csv('/home/kmc249/Downloads/newest_psf_fluxes_neighborhood_2008_apphot.csv', index=False)
