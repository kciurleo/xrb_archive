...
basedir="/neta/xrb/AqlX-1/1.3m/ir/proc/H"
...
base='/neta/xrb/AqlX-1/1.3m/ir/proc/H/night_081031/final_stack_aligned_to_081031.fits'
...
for id, file in enumerate(filelist):
    ...
    if len(parts) >= 2:
        ...
        print('bad folder format', folder)
        continue
    try:
        dt = pd.to_datetime(date_str, format="%y%m%d")
        mjd = Time(dt).mjd
    except:
        print('error with datestr', date_str)
        continue
    ...
    try:
        ...
        print('no file', file)
        try:
            im,hdr = fits.getdata(f'{file.split(".")[0]}_H.fits',header=True)
        except:
            print('def no file')
            continue
    try:
...
fwhms.to_csv(f'{savedir}H_ir_countrates.csv', index=False)

fwhms=pd.read_csv(f'{savedir}H_ir_countrates.csv', low_memory=False)
...
fits.writeto(f'{savedir}Aql_H_stack_NEW_MEDIAN_SCALED.fits',median_image,header=HDR, overwrite=True)
winners.to_csv(f'{savedir}Aql_H_list_MEDIAN.csv', index=False)
